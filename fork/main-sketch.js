...
function setup() {
  ...
  let mainMountainStartY =
    windowHeight * 0.5 + fxRandomRange(-0.3, 0.3) * windowWidth;
  ...
  for (let i = 0; i < totalMountainNum; i++) {
    ...
    if (i < upMountains) {
      ...
      startY =
        mainMountainStartY +
        mountainOffsetUnit * offsetValue +
        fxRandomRange(-0.1, 0.1) * windowWidth;
...
class MountainCurve {
  ...
  calculateY(x) {
    ...
    return (
      allMountainOffset +
      this.startY +
      (noise(noiseValue) - 0.5) * this.mountainHeight
    );
  }
}
