...
import { random, randomFloor, getRandomElement } from "./utils";
...
class SceneApp extends Scene {
  constructor() {
    ...
    if (!NY_isCollage) startRender = true;
  }

  _sendCoordinate() {
    // get left Y / right Y
    const { startY, endY } = getStartEnd(this._fboRenderBg);
    console.log("startY, endY", startY, endY, NY_mountainIndex);
    // let search = new URLSearchParams(window.location.search);
    // let NY_mountainIndex = search.get("mountainIndex");
    const fullHeight = GL.height;
    const artistId = 1;
    const featureValue = random(10);
    let message = {
      event: "ready",
      args: {
  ...
  _initViews() {
    ...
    if (random() > 0.5) {
      ...
      while (colors.length < 5) {
        colors.push(getRandomElement(colors));
      }
    }
  ...
  render() {
    ...
    this._drawCompose
      ...
      .uniform("uMonochrome", Config.monochrome ? 1.0 : 0.0)
